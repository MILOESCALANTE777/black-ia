...
const options = [
  ...
  { key: 'futures', label: 'Futures', emoji: '⏳' },
  ...
  return (
    <div className="flex-1 flex flex-col bg-black px-6">
      <div className="pt-6 flex items-center gap-4">
        <button onClick={() => navigate('ExperienceSelectScreen')} className="p-2 -ml-2">
          <ChevronLeft size={24} color="#FFFFFF" />
        </button>
        <div className="flex-1 h-1 bg-[#38383A] rounded-full overflow-hidden">
          <motion.div initial={{ width: '25%' }} animate={{ width: '50%' }} className="h-full bg-white rounded-full" />
        </div>
        <div className="w-10" />
      </div>
      ...
      <div className="flex-1 flex flex-col justify-center gap-3 mt-6">
        {options.map((opt, i) => (
          <motion.button
            ...
            className="w-full py-4 rounded-2xl text-center text-lg font-semibold transition-all duration-200 active:scale-[0.98]"
            style={{
              background: selected === opt.key ? '#2C2C2E' : '#1C1C1E',
              border: `1.5px solid ${selected === opt.key ? '#FFFFFF' : '#38383A'}`,
              color: opt.key === 'other' && selected !== opt.key ? '#636366' : '#FFFFFF',
            }}
          >
            {opt.label} {opt.emoji}
          </motion.button>
